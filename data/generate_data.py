...
contract = {
    "date": dates,
    "oct": [100 + np.random.normal(-1, 1) for _ in dates],
    "nov": [100 + np.random.normal(0, 3) for _ in dates],
    "dec": [100 + np.random.normal(-1, 1) for _ in dates],
}
...
df.to_csv("data/generated_daily_values.csv", index=False)
